...
void Test1(unsigned int size) {
  ...
  {
	  char *s=new char[l];
	  sprintf(s, "Exception MemoryNotAllocated: Too much memory for allocation in operator new (size=%u)",size);
...
void Test2() {
  for (int i = 0; i < EXP_TEST2_COUNT; i++) {
	...
	{
		char *s=new char[l];
		sprintf(s,"Exception DivisionByZero: MyDiv with arguments (x=%lf) and (y=%lf)", x, y);
...
void Test3(A *b) {
	...
	{
		char *s=new char[l];
		sprintf(s, "Exception BadTypeCast: Error in dynamic_cast.");
...
double Sum(long double n) 
{
  ...
  {
	  char *s=new char[l];  
	  sprintf(s,"Exception DivisionByZero: Argument is zero in function Sum");  
	  throw Exception3(s,0);  
  }  
  try {  
	  return 1./n + Sum(n - 1);  
  ...
  {
	  char*s=new char[l];
	  sprintf(s,"Exception WrongArgument:(n=%lf)",n);
